/*******************************************************************************
* ARM assembly implementation of fixsliced SKINNY-128.
*
...
/******************************************************************************
* Applies P^2 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p2:
	movw 	r1, #0xcc00
...
/******************************************************************************
* Applies P^4 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p4:
...
/******************************************************************************
* Applies P^6 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p6:
...
/******************************************************************************
* Applies P^8 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p8:
...
/******************************************************************************
* Applies P^10 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p10:
	movw 	r12, #0x0033
...
/******************************************************************************
* Applies P^12 on a tweakey state in a bitsliced manner
******************************************************************************/
.align 	2
p12:
	str.w 	r14, [sp] 					//store r14 on the stack
...
/******************************************************************************
* Apply P^14 on a tweakey state in a bitsliced manner.
******************************************************************************/
.align 	2
p14:
	movw 	r1, #0xcc00
...
/******************************************************************************
* Pack the input into the fixsliced representation.
******************************************************************************/
.align 2
packing:
	eor 	r12, r2, r2, lsr #3
...
/******************************************************************************
* Unpack from fixsliced to byte array representation.
******************************************************************************/
.align 2
unpacking:
	movw 	r6, #0x0a0a
	movt 	r6, #0x0a0a 			// mask for SWAPMOVE
	eor 	r10, r3, r5, lsr #2
...
******************************************************************************/
@ void 	tkschedule_lfsr_3(u32* tk, const u8* key, const int rounds)
...
tkschedule_lfsr_3:
	...
	ldr.w 	r1, [sp, #8] 				//load loop counter in r1
	stmia 	r0!, {r2-r5}
	loop_3:
...
******************************************************************************/
@ void 	tkschedule_lfsr_2(u32* tk, const u8* key, const int rounds)
...
tkschedule_lfsr_2:
	...
	loop_2:
		...
		eor 	r11, r2, r7 			//tk2 ^ tk3 (1st word)
		eor 	r12, r3, r8 			//tk2 ^ tk3 (2nd word)
		strd 	r11, r12, [r0], #8 		//store in tk
		eor 	r11, r4, r9 			//tk2 ^ tk3 (3rd word)
		eor 	r12, r5, r6				//tk2 ^ tk3 (4th word)
		...
		eor 	r11, r2, r8 			//tk2 ^ tk3 (1st word)
		eor 	r12, r3, r9 			//tk2 ^ tk3 (2nd word)
		strd 	r11, r12, [r0], #8 		//store in tk
		eor 	r11, r4, r6 			//tk2 ^ tk3 (3rd word)
		eor 	r12, r5, r7				//tk2 ^ tk3 (4th word)
		strd 	r11, r12, [r0], #24 	//store in tk
		ldm 	r0, {r2-r5} 			//load tk3
		...
		eor 	r11, r2, r9 			//tk2 ^ tk3 (1st word)
		eor 	r12, r3, r6 			//tk2 ^ tk3 (2nd word)
		strd 	r11, r12, [r0], #8 		//store in tk
		eor 	r11, r4, r7 			//tk2 ^ tk3 (3rd word)
		eor 	r12, r5, r8				//tk2 ^ tk3 (4th word)
		strd 	r11, r12, [r0], #24 	//store in tk
		ldm 	r0, {r2-r5} 			//load tk3
...
* Performing both at the same time allows to save some memory access
* (~400 cycles faster than tkschedule_lfsr_2 + tkschedule_lfsr_3)
******************************************************************************/
...
/******************************************************************************
* Given rtk = LFSR2(TK2) ^ LFSR3(TK3) and tk1 = TK1, returns 
* P(TK1 ^ LFSR2(TK2) ^ LFSR3(TK3)) for all round tweakeys.
* The function is fully unrolled to add the correct rconst in the fixsliced
* representation. Depending on the 'rounds' input parameter, the function may
* ends sooner (for SKINNY-128-128 and SKINNY-128-256).
******************************************************************************/
@ void 	tkschedule_perm(u32* rtk, const u8* tk1, const int rounds)
...
tkschedule_perm:
	...
	eor 	r6, r6, r2 					//tk <- tk1 ^ tk2 ^ tk3 (1st word)
	eor 	r7, r7, r3 					//tk <- tk1 ^ tk2 ^ tk3 (2nd word)
	eor 	r8, r8, r4 					//tk <- tk1 ^ tk2 ^ tk3 (3rd word)
	eor 	r9, r9, r5 					//tk <- tk1 ^ tk2 ^ tk3 (4th word)
	...
	and 	r11, r10, r6, ror #30 		//--- ror and masks to match fixslicing
	and 	r12, r6, r10, ror #4
	orr 	r12, r11, r12, ror #22
	eor 	r12, r12, #0x00000010
	str.w 	r12, [r0], #4
	and 	r11, r10, r7, ror #30
	and 	r12, r7, r10, ror #4
	orr 	r12, r11, r12, ror #22
	str.w 	r12, [r0], #4
	and 	r11, r10, r8, ror #30
	and 	r12, r8, r10, ror #4
	orr 	r12, r11, r12, ror #22
	eor 	r12, r12, #0x00010000
	eor 	r12, r12, #0x00000010
	str.w 	r12, [r0], #4
	and 	r11, r10, r9, ror #30
	and 	r12, r9, r10, ror #4
	orr 	r12, r11, r12, ror #22		//ror and masks to match fixslicing ---
	eor 	r12, r12, #0x00000400
	mvn 	r12, r12 					//to save a NOT in sbox calculations
	str.w 	r12, [r0], #4
	ldr.w 	r11, [sp, #12] 				//load the 'rounds' input parameter
	subs 	r11, #40 					//if only 40 rounds (SKINNY-128-128)
	ble 	exit 						//then exit
	orr 	r10, r10, r10, lsl #2 		//r10<- 0xf0f0f0f0
	and 	r6, r10, r6 				//ror and mask to match fixslicing
	and 	r7, r10, r7 				//ror and mask to match fixslicing
	eor 	r7, r7, #0x00000040 		//add rconst
	strd 	r6, r7, [r0, #8] 			//store 2nd half tk for 41th round
	and 	r8, r10, r8 				//ror and mask to match fixslicing
	and 	r9, r10, r9 				//ror and mask to match fixslicing
	eor 	r8, r8, #0x00000014 		//add rconst
	eor 	r9, r9, #0x00000010 		//add rconst
	mvn 	r9, r9 						//to save a NOT in sbox calculations
	strd 	r8, r9, [r0], #16 			//store 1st half tk for 41th round
	ldm 	r0, {r6-r9} 				//load tk
	eor 	r6, r6, r2 					//tk <- tk1 ^ tk2 ^ tk3 (1st word)
	eor 	r7, r7, r3 					//tk <- tk1 ^ tk2 ^ tk3 (2nd word)
	eor 	r8, r8, r4 					//tk <- tk1 ^ tk2 ^ tk3 (3rd word)
	...
	str.w 	r12, [r0], #4
	ldr.w 	r11, [sp, #12] 				//load the 'rounds' input parameter
	subs 	r11, #48 					//if only 48 rounds (SKINNY-128-256)
	ble 	exit 						//then exit
	orr 	r10, r10, r10, lsl #2 		//r10<- 0xf0f0f0f0
	and 	r6, r10, r6 				//ror and mask to match fixslicing
	and 	r7, r10, r7 				//ror and mask to match fixslicing
	eor 	r6, r6, #0x00000040 		//add rconst
	strd 	r6, r7, [r0, #8] 			//store 2nd half tk for 49th round
	and 	r8, r10, r8 				//ror and mask to match fixslicing
	and 	r9, r10, r9 				//ror and mask to match fixslicing
	eor 	r8, r8, #0x00000004 		//add rconst
	eor 	r9, r9, #0x00000040 		//add rconst
	mvn 	r9, r9 						//to save a NOT in sbox calculations
	strd 	r8, r9, [r0], #16 			//store 1st half tk for 49th round
	ldm 	r0, {r6-r9} 				//load tk
	eor 	r6, r6, r2 					//tk <- tk1 ^ tk2 ^ tk3 (1st word)
	eor 	r7, r7, r3 					//tk <- tk1 ^ tk2 ^ tk3 (2nd word)
	eor 	r8, r8, r4 					//tk <- tk1 ^ tk2 ^ tk3 (3rd word)
	...
	exit: 								//label to exit sooner for 128 & 256
		add.w 	sp, #4
		pop 	{r0-r12, lr}
		bx 		lr

/******************************************************************************
* Quadruple round of fixsliced SKINNY-128.
******************************************************************************/
.align 	2
quadruple_round:
	orr 	r8, r2, r3
...
******************************************************************************/
@ void 	skinny128_128(u8* ctext, const u32* tk, const u8* ptext)
.global skinny128_128
.type   skinny128_128,%function
.align 2
skinny128_128:
	push 	{r1-r12, r14}
	ldr.w 	r3, [r2, #8]
	...
	ldr.w 	r2, [r2]
	movw 	r10, #0x0a0a
	movt 	r10, #0x0a0a 			// mask for SWAPMOVE
	movw 	r11, #0x3030
	movt 	r11, #0x3030 			// mask for SWAPMOVE
	bl 		packing
	movw 	r6, #0x5555
	movt 	r6, #0x5555 			// mask for SWAPMOVE
	mov 	r7, r11
	bl 		quadruple_round
	bl 		quadruple_round
	bl 		quadruple_round
...
/******************************************************************************
* Encrypt a single block using fixsliced SKINNY-128-256.
******************************************************************************/
@ void 	skinny128_256(u8* ctext, const u32* tk, const u8* ptext)
.global skinny128_256
.type   skinny128_256,%function
.align 2
skinny128_256:
	push 	{r1-r12, r14}
	ldr.w 	r3, [r2, #8]
...
* Encrypt a single block using fixsliced SKINNY-128-128.
******************************************************************************/
@ void 	skinny128_128_inv(u8* ptext, const u32* rtk, const u8* ctext)
.global skinny128_128_inv
.type   skinny128_128_inv,%function
.align 2
skinny128_128_inv:
	push 	{r1-r12, r14}
	ldr.w 	r3, [r2, #8]
	...
	movt 	r6, #0x5555 			// mask for SWAPMOVE
	mov 	r7, r11
	add 	r1, #632 				// points to the last round tweakey
...
* Encrypt a single block using fixsliced SKINNY-128-256.
******************************************************************************/
@ void 	skinny128_256_inv(u8* ptext, const u32* tk, const u8* ctext)
.global skinny128_256_inv
.type   skinny128_256_inv,%function
.align 2
skinny128_256_inv:
	push 	{r1-r12, r14}
	ldr.w 	r3, [r2, #8]
	...
	movt 	r6, #0x5555 			// mask for SWAPMOVE
	mov 	r7, r11
	add 	r1, #760 				// points to the last round tweakey
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
...
* Encrypt a single block using fixsliced SKINNY-128-384.
******************************************************************************/
@ void 	skinny128_384_inv(u8* ptext, const u32* tk, const u8* ctext)
...
skinny128_384_inv:
	...
	movt 	r6, #0x5555 			// mask for SWAPMOVE
	mov 	r7, r11
	add 	r1, #888 				// points to the last round tweakey
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
