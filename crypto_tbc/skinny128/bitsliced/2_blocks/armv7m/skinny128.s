/*******************************************************************************
* ARM assembly fixsliced implementation of the SKINNY-128 block ciphers.
...
/******************************************************************************
* Packs TK1 and copies the resulting fixsliced representation into the round
* tweakey array.
******************************************************************************/
@ void 	pack_tk1(u32* rtk, const u8* tk1,  const u8* tk2_bis, const int rounds)
.global pack_tk1
.type   pack_tk1,%function
.align	2
pack_tk1:
	push 	{r0-r12, r14}
	...
	movw 	r2, #0x5555
	movt 	r2, #0x5555 			// masks for SWAPMOVE
	movw 	r3, #0x3030
	movt 	r3, #0x3030 			// masks for SWAPMOVE
	bl 		packing
	stmia 	r0!, {r5-r12} 			// store tk2 in rtk after packing
	loop_1:
		stmia 	r0!, {r5-r12}
		add 	r0, #32
...
tkschedule_lfsr_2:
	...
	movt 	r3, #0x3030 			//r3 <- 0x30303030
	bl 		packing
	ldm 	r0, {r2-r4,r14} 		// load rtk (computed by pack_tk1)
	eor 	r2, r2, r5 				// rtk <- tk1 ^ tk2
	eor 	r3, r3, r6 				// rtk <- tk1 ^ tk2
	eor 	r4, r4, r7 				// rtk <- tk1 ^ tk2
	eor 	r14, r14, r8 			// rtk <- tk1 ^ tk2
	stmia 	r0!, {r2-r4,r14} 		// store rtk after adding tk3
	ldm 	r0, {r2-r4,r14} 		// load rtk (computed by pack_tk1)
	eor 	r2, r2, r9 				// rtk <- tk1 ^ tk2
	eor 	r3, r3, r10 			// rtk <- tk1 ^ tk2
	eor 	r4, r4, r11 			// rtk <- tk1 ^ tk2
	eor 	r14, r14, r12 			// rtk <- tk1 ^ tk2
	stmia 	r0!, {r2-r4,r14} 		// store rtk after adding tk3
	loop_2:
		eor 	r5, r5, r7			// apply LFSR2 to tk2
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r6 			// rtk <- tk1 ^ tk2
		eor 	r3, r3, r7 			// rtk <- tk1 ^ tk2
		eor 	r4, r4, r8 			// rtk <- tk1 ^ tk2
		eor 	r14, r14, r9 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r10 		// rtk <- tk1 ^ tk2
		eor 	r3, r3, r11 		// rtk <- tk1 ^ tk2
		eor 	r4, r4, r12 		// rtk <- tk1 ^ tk2
		eor 	r14, r14, r5 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		...
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r7 			// rtk <- tk1 ^ tk2
		eor 	r3, r3, r8 			// rtk <- tk1 ^ tk2
		eor 	r4, r4, r9 			// rtk <- tk1 ^ tk2
		eor 	r14, r14, r10 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r11 		// rtk <- tk1 ^ tk2
		eor 	r3, r3, r12 		// rtk <- tk1 ^ tk2
		eor 	r4, r4, r5 			// rtk <- tk1 ^ tk2
		eor 	r14, r14, r6 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		add 	r0, r0, #32 		// same round tweakey every 2 rounds
		subs 	r1, r1, #2 			// decrease loop counter by 2
		beq 	exit_lfsr 			// if 0 then we are done
		...
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r8 			// rtk <- tk1 ^ tk2
		eor 	r3, r3, r9 			// rtk <- tk1 ^ tk2
		eor 	r4, r4, r10 		// rtk <- tk1 ^ tk2
		eor 	r14, r14, r11 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		ldm 	r0, {r2-r4,r14} 	// load rtk (computed by tkschedule_lfsr_2)
		eor 	r2, r2, r12 		// rtk <- tk1 ^ tk2
		eor 	r3, r3, r5 			// rtk <- tk1 ^ tk2
		eor 	r4, r4, r6 			// rtk <- tk1 ^ tk2
		eor 	r14, r14, r7 		// rtk <- tk1 ^ tk2
		stmia 	r0!, {r2-r4,r14} 	// store rtk after adding tk3
		add 	r0, r0, #32 		// same round tweakey every 2 rounds
		subs 	r1, r1, #2 			// decrease loop counter by 2
		beq 	exit_lfsr 			// if 0 then we are done
...
/******************************************************************************
* Applies the permutation to all the round tweakeys and add the round constants
* according the fixsliced representation.
******************************************************************************/
@ void 	tkschedule_perm(u32* rtk, const int rounds)
...
tkschedule_perm:
	...
	mvn 	r3, r3 					// to save 1 NOT in Sbox calculations
	strd 	r1, r3, [r0], #8
	ldr.w 	r1, [sp, #8] 			// load the input parameter 'rounds'
	subs 	r1, #40 				// if rounds == 40 (i.e. SKINNY-128-128)
	ble 	exit 					// then we are done
	orr 	r4, r4, r4, lsl #2 		// r4 <- 0xf0f0f0f0
	and 	r5, r5, r4 				// tk &= 0xf0f0f0f0 (extract rows 1&2 only)
	and 	r6, r6, r4 				// tk &= 0xf0f0f0f0 (extract rows 1&2 only)
	and 	r7, r7, r4 				// tk &= 0xf0f0f0f0 (extract rows 1&2 only)
	...
	strd 	r1, r3, [r0], #8
	ldr.w 	r1, [sp, #8] 			// load the input parameter 'rounds'
	subs 	r1, #48 				// if rounds == 48 (i.e. SKINNY-128-256)
	ble 	exit 					// then we are done
	orr 	r4, r4, r4, lsl #2 		// r4 <- 0xf0f0f0f0
	and 	r5, r5, r4 				// tk &= 0xf0f0f0f0 (extract rows 1&2 only)
	and 	r6, r6, r4 				// tk &= 0xf0f0f0f0 (extract rows 1&2 only)
...
/******************************************************************************
* Quadruple round for fixsliced SKINNY-128 tweakable block cipher.
...
quadruple_round:
	...
	ldmia.w r1!, {r0,r2-r4} 		//load rkeys in r0,r2,r3,r4
	eor 	r5, r5, r0 				//add rkey + rconst
	eor 	r6, r6, r2 				//add rkey + rconst
	eor 	r7, r7, r3 				//add rkey + rconst
	eor 	r8, r8, r4 				//add rkey + rconst
	ldmia.w r1!, {r0,r2-r4} 		//load rkeys in r0,r2,r3,r4
	eor 	r9, r9, r0 				//add rkey + rconst
	eor 	r10, r10, r2 			//add rkey + rconst
	eor 	r11, r11, r3 			//add rkey + rconst
	eor 	r12, r12, r4 			//add rkey + rconst
	movw 	r2, #0x3030
	...
	// computes the sbox output bit permutation every 4 rounds
...
/******************************************************************************
* Encrypt 2 blocks at a time using SKINNY-128-128, given all precomputed round
* tweakeys.
******************************************************************************/
@ void skinny128_128(u8* ctext, u8* ctext_bis, const u8* ptext,
...
skinny128_128:
	...
	ldr.w 	r1, [sp, #56] 			// load rtk addr from the stack (5th arg)
	movw 	r2, #0x5555
	movt 	r2, #0x5555 			// mask for SWAPMOVE
	movw 	r3, #0x3030
	movt 	r3, #0x3030 			// mask for SWAPMOVE
	bl 		packing
	bl 		quadruple_round
	bl 		quadruple_round
	bl 		quadruple_round
...
/******************************************************************************
* Encrypt 2 blocks at a time using SKINNY-128-256, given all precomputed round
* tweakeys.
******************************************************************************/
@ void skinny128_256(u8* ctext, u8* ctext_bis, const u8* ptext,
@						const u8* ptext_bis,  const u32* rtk)
...
/******************************************************************************
* Encrypt 2 blocks at a time using SKINNY-128-384, given all precomputed round
* tweakeys.
******************************************************************************/
...
* tweakeys.
******************************************************************************/
@ void skinny128_128_inv(u8* ctext, u8* ctext_bis, const u8* ptext,
@						const u8* ptext_bis,  const u32* rtk)
...
skinny128_128_inv:
	...
	movt 	r3, #0x3030 			// mask for SWAPMOVE
	bl 		packing
	add 	r1, r1, #1272 			// points to last round tweakey
...
* tweakeys.
******************************************************************************/
@ void skinny128_256_inv(u8* ctext, u8* ctext_bis, const u8* ptext,
@						const u8* ptext_bis,  const u32* rtk)
...
skinny128_256_inv:
	...
	movt 	r3, #0x3030 			// mask for SWAPMOVE
	bl 		packing
	add 	r1, #1528 				// points to last round tweakey
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
...
skinny128_384_inv:
	...
	movt 	r3, #0x3030 			// mask for SWAPMOVE
	bl 		packing
	add 	r1, #1784 				// points to last round tweakey
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
	bl 		inv_quadruple_round
